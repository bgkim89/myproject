...
if not df_filtered.empty:
    df_filtered['연도'] = pd.to_datetime(df_filtered['시간']).dt.year
    df_filtered['규모'] = df_filtered['규모'].astype(float)
    ...
    with tab2:
        fig2 = px.histogram(df_filtered, x='규모', nbins=20, title="지진 규모 분포 (히스토그램)")
        st.plotly_chart(fig2, use_container_width=True)
else:
    st.info("검색 결과가 없습니다. 다른 지역명을 입력해보세요.")
